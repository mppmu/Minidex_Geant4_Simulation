  ...
  //--> first clear up
   ...
   //--> second loop over all hits and sort them according to scintillator or hpge hits
    for (int ihit=0; ihit<hits_totnum; ihit++) {
        if ((*hits_edep)[ihit]<=0.001) continue; // hits with energy eV or below are not processed (in simulation there are many hits with e-05 eV energy)
   ...
   //--> gross scintillator and hpge energy and time without time-clustering
    for (int iscin=0; iscin<16; iscin++) {
...
//     now, the seed time is simply the time from the hit with the by-far largest energy 
...
*/
  ...
  //--> second, clear up all bufferevt variables;
    for (int ievt=0; ievt<1000; ievt++) {
       bufferevt_time[ievt]=0.0;
       for (int i=0; i<16; i++) bufferevt_edep[ievt][i]=0.0;
    }
...
void HPGeEventData::FindPrimaryMuonTagTypeID()
{
   ...
   double muontag_ethreshold=2000.0;
   double muontag_evetothreshold=3000.0;
...
//---> start looping over all scinevt
   for (int imu=0; imu<scinevt_time->size(); imu++) {
     for (int iscint=0; iscint<16; iscint++) Muon_E[iscint]=0.0;
   ...
   //---> muonthrough-1b1
    ...
    //---> index
       if (primary_muontag_typeid>=0) primary_muontag_index=imu;
