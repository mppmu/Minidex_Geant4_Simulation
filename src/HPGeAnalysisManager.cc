...
*/
   // hpge and scintillator energies (sum of hits energies), 2-dim variable, NOT 2-dim vector yet
    ...
    m_pTree->Branch("scintillator_tote",&m_pEventData->scintillator_tote,"scintillator_tote[16]/D");
    m_pTree->Branch("scintillator_time",&m_pEventData->scintillator_time,"scintillator_time[16]/D");
/*
...
//---> hits in Germanium detector
   ...
   for (int icha=0; icha<6; icha++) m_pEventData->scintillator_time[icha]=0.0;
    ...
    for (G4int i=0; i<pHPGeHitsCollection->entries(); i++) {
      ...
      if (pHit->GetEnergyDeposited()>0.0) {  // save only hits with positive deposited energy
